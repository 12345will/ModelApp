...
# MATERIAL DATA (PLACEHOLDERS)
materials_data = {
    "NMC Cell 1": {
        "materials": {"NCM": 0.5, "SP-01": 0.5, "PVDF-1": 0.5, "NMP": 0.5, "Boehmite": 0.5,
    ...
    "NMC Cell 2": {
        "materials": {"NCM - CAM powder": 0.2, "SP-01": 0.2, "PVDF-1": 0.2, "NMP": 0.2, "Boehmite": 0.2,
    ...
    "LFP": {
        "materials": {"Polypropylene": 0.3, "Aluminium Foil": 0.3, "NMP Solvent - Cathode": 0.3,
...
for year in YEARS:
    ...
    uk_energy_used = uk_co2 = uk_water = 0
    uk_materials = {}
    if uk_valid and uk_energy > 0:
        uk_energy_used, uk_cells, uk_co2, uk_water, uk_materials = calc_site_from_energy(
            uk_energy, uk_cells,
    ...
    in_energy_used = in_co2 = in_water = 0
    in_materials = {}
    if in_valid and in_energy > 0:
        in_energy_used, in_cells, in_co2, in_water, in_materials = calc_site_from_energy(
            in_energy, in_cells,
            {"NMC Cell 1": in_mix_nmc1, "NMC Cell 2": in_mix_nmc2, "LFP": in_mix_lfp},
            in_silicon
        )

    total_energy = uk_energy_used + in_energy_used
    total_cells = uk_cells + in_cells
    total_co2 = uk_co2 + in_co2
    total_water = uk_water + in_water
    total_cost = (uk_energy_used * 1e6 * UK_PRICE_PER_KWH) + (in_energy_used * 1e6 * INDIA_PRICE_PER_KWH)
...
st.download_button("📥 Download Annual Results CSV", df.to_csv(index=False), "annual_results.csv")
...
if annual_materials_list:
    ...
    st.download_button("📥 Download Annual Material Usage CSV", annual_materials_df.to_csv(index=False), "annual_materials.csv")
else:
    st.info("No material data yet.")

st.subheader("📈 Cumulative Totals")
cum = df.drop(columns=["Year"]).sum(numeric_only=True).to_frame(name="Total").T
st.dataframe(cum.style.format({
    "Total Cells": "{:,.0f}",
    "Total Energy (GWh)": "{:,.2f}",
    "Total CO2 (tCO2)": "{:,.0f}",
    "Total Water (m³)": "{:,.0f}",
...
if cumulative_materials:
    ...
    st.download_button("📥 Download Cumulative Material Usage CSV", cum_mat_df.to_csv(index=False), "cumulative_materials.csv")
